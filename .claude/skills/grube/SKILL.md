...
- **Single-size produkty (nože) NEMAJÚ Offer-list** — jediný itemId je len v `itemId=<id>` kotve stránky (reminder link + canonical fragment `…/<pid>/#itemId=<id>`). **#60 class 1 HOTOVÉ (v0.79.0):** `parse_variants` pri prázdnom Offer-liste vezme vlastný itemId z `itemId=` kotvy (`prefix==productId AND len==pid+4`, rovnaký filter ako sku); PRESNE 1 → `{ONE_SIZE: itemId}` (`ONE_SIZE=""` sentinel), 0/>1 → `{}` (link-only, fail-closed). `match_variant_codes`: one-size row (`resolve_size None`) sa napáruje na single-size grube LEN keď je 1 one-size row (nikdy 1 itemId → N kódov).
- **#60 class 3 (rozsahové / single / cm) HOTOVÉ (v0.82.0) — BEZ zmeny kódu, exact-match už funguje.** Naživo overené (Playwright, 2026-07-23): grube.de píše v Offer `name` IDENTICKÝ veľkostný string ako forestshop pre KAŽDÝ „špeciálny" formát — slash rozsahy `Größe 39/40.` == `39/40` (košeľa pid 113567), single čísla `Größe 39.` == `39` (gumáky pid 134030), cm detské `Größe 140.` == `140` (pid 115162). `Gr. X` je LEN v popise/Größentabelle, NIKDY v Offer name. Preto EXACT-string match pokrýva class 3 — **žiadna normalizácia netreba** (predpoklad spec-u „grube = `Gr. 39-40`" je NEPRAVDIVÝ). **NEROB fuzzy range-membership** (`39` → `39/40`) — je zakázané (fail-closed: `39` môže byť topánka, nie golier). Fixtúra `grube_de_detail_113567_kosela.html`. Fill po gathери: pid 113567 (5) + gumáky 134030/907862 + detské 115162/751627 + baretka 1319.
- **#60 class 2 (dvojosové komplety bunda+nohavice) = link-only NAVŽDY, bezpečne sa NEDÁ inak.** Naživo overené: grube.de predáva bundu a nohavice ako SAMOSTATNÉ jednoosové produkty (Grand Nord Latzhose pid 604557 = `Größe 46`…`64`); žiadny grube produkt nemá dvojosový (`Größe Jacke × Größe Hose`) Offer, a ani jeden forestshop komplet (pair 1234 Grand Nord / 1235 Tradition, 134 var.) NIE je napárovaný (AI kandidáti sú separátne kusy). Jeden komplet variant = jedna SKU cez DVA grube produkty; `externalCode` je JEDNO pole → nedá sa reprezentovať. `resolve_size` → **MULTI_AXIS** → skip = jediný správny fail-closed výsledok. **#60 UZAVRETÉ** (v0.82.0, všetky 3 triedy).
