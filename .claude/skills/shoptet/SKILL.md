...
- **n8n workflow „Forestshop — Párovania → eshop"** (`YuDugCCOnwejRfva`): `Denne 21:00 → HTTP „Nahraj parovania" → Sprava → Discord`. Sprava cappuje na 25 produktov + chunkuje + `<>`-obaľuje linky; `count:0` → `return []` → žiadna Discord správa.
...
- **PASCA — result read-back pri CHYBE/async importe je nespoľahlivý:** keď import padne na chyby (alebo je veľmi veľký/async), `_read_result` chytí STARÝ výsledok (z minulého behu) → endpoint nahlási falošný úspech. Po dedupe sa to nedeje (čistý import = reálny výsledok), ale hardening je v issue #23. **Over reálne cez čerstvý export** (`internalNote` je stĺpec 257 v pattern-14 CSV) — nie len cez nahlásený výsledok.
