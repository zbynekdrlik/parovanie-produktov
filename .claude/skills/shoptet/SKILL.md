...
- **Ako zistiť, čo urobí najbližší nočný beh — BEZ zápisu do eshopu a bez dotyku živých dát:** skopíruj `data/out/{decisions,order_pairings,uploaded_pairings,review_data,supplier_assignments}.json` do tmp adresára, `WEBREVIEW_OUT=<tmp>` (`WEBREVIEW_PRODUCTS` môže ukazovať na živý `data/products.csv` — číta sa len na čítanie), importuj `webreview/app.py` a poskladaj riadky cez `import_builder.new_pairing_keys` / `link_rows` / `new_order_pairing_keys` / `order_pairing_rows` + `_codes_confirmed_in_export`. Dostaneš presné čísla (koľko riadkov, koľko potvrdí export, koľko sa reálne odošle, v koľkých častiach) bez jediného volania `run_import`. **`data/out` je ŽIVÁ práca manažéra — nikdy doň nepíš z analýzy ani z testu** (backend testy to majú poistené v `tests/conftest.py`, ktorý prepne `WEBREVIEW_OUT`/`WEBREVIEW_PRODUCTS` na tmp ešte pred zberom testov).
...
- **Potvrdenie z exportu má DÁTUM SPOTREBY (`EXPORT_MAX_AGE_S`, 6 h).** Potvrdený riadok sa NEposiela a ZAPÍŠE sa ako nahraný — čiže je len taký dôveryhodný ako `data/products.csv`, ktorý obnovuje INÁ hodinová automatizácia. Keď ten sync stojí, súbor si drží staré bajty a kód medzitým zmenený v eshope by sa už nikdy neprepísal. Starší export → nepotvrdiť nič, radšej riadky poslať (idempotentné). Chýbajúci súbor blokovať netreba — vtedy `_read_export_for_links` aj tak vráti `""`. **Export sa preto číta VNÚTRI `_codes_confirmed_in_export`, hneď za kontrolou veku — funkcia zámerne neprijíma predpripravené `notes=`**: akonáhle by sa dali podať zvonku (prirodzená optimalizácia „prečítaj export raz a podaj ďalej"), kredit by vznikol z bajtov, ktorých vek nikto neoveril, a táto poistka by ticho zmizla.
