...
- **Discord = DETAILNÝ zoznam** (nie len počet): Code uzol „Sprava" zoskupí kandidátov podľa produktu (`$('Vyhodnoť kandidátov').all()`) a zlepí markdown — `N produktov (M variantov)`, per produkt **názov (veľkosti) — cena (konk. cena)** + forestshop URL na overenie, na konci **📦 Import: spracované/upravené/zlyhania** (z `$('Importuj do Shoptetu').first().json`). Discord `content = ={{ $json.text }}`, bez prílohy, cap 1950 znakov (Discord limit 2000). Tak vidno KOHO naskladnilo a dá sa klikom overiť.
