...
**VEĽKÝ import → OBÁVAJ SA 120s browser timeoutu, chunkuj (#156):** `scripts/shoptet_import.py:296` čaká `page.wait_for_url(re.compile(r"import-produktov/log"), timeout=120000)` — Shoptet spracúva veľké CSV na SERVERI dlho a presmeruje na log až potom. **~1195 riadkov > 120 s → zlyhanie „Timeout 120000ms".** Root-cause fix (NIE väčší timeout — `no-timeout-band-aids`): **rozdeľ dávku na časti ≤`IMPORT_CHUNK_ROWS` (300) riadkov**, jeden `run_import` na časť. V `webreview/app.py` je na to zdieľaný **`_import_rows_chunked(all_rows, header, dry, prefix, csv_safe=?, timeout=900)`** — volaj ho pod `_import_lock` (drž lock cez CELÚ dávku), release v `finally` (žiaden zaseknutý lock). PRVÁ **tvrdo** zlyhaná časť zastaví dávku (čiastočne prijatá časť dávku NEzastaví — viď `chunk_outcome` nižšie); `success_codes` = kódy z ČISTÝCH častí, `partial_codes` = kódy z čiastočne prijatých (tie sa NEsmú zapísať ako nahraté — nevieme ktoré prešli) → uploaded stav zapíš LEN pre kľúče, ktorých VŠETKY kódy sú v `success` (idempotent + resumable + #49 ochrana partial-key). Hlášku o nedokončenej dávke stavaj cez `_chunk_error_msg(res, total_rows)`. **(#299 záverečná recenzia, Minor)** Po migrácii na `pending_shoptet` (Task 10) `_import_rows_chunked` reálne volajú len DVE cesty — `/api/n8n/shoptet-import` (surový n8n restock feed) a hodinový drain **`run_shoptet_upload`** — over si to `grep -n "_import_rows_chunked(" webreview/app.py` pred písaním nového volajúceho, nespoliehaj sa na počet v tomto texte. `_do_upload_pairings`, `_do_upload_suppliers`, `run_restock_skladom`/`run_stock_skladom` (#299 Task 9/10) do Shoptetu už PRIAMO nezapisujú — len QUEUE-ujú riadky (`queue_shoptet_fields`) do `pending_shoptet`, ktoré `run_shoptet_upload` zoberie a pošle cez tento chunker; žiaden z nich necháva jeden veľký `run_import` na celú dávku, lebo ani jeden `run_import` priamo nevolá. `_chunk_error_msg` má dnes tiež len dvoch volajúcich — tých istých dvoch, čo volajú `_import_rows_chunked`. `n8n_shoptet_import` má drobnú odlišnosť: `sanitize_csv(raw_path, out_path)` najprv zapíše CELÚ sanitizovanú dávku ako audit súbor (`out_path`, prežíva aj po behu), a AŽ POTOM sa `out_path` prečíta späť (`csv.reader`) na riadky pre `_import_rows_chunked` — nečíta sa surový upload dvakrát, ale kanonický už-sanitizovaný súbor.
...
- **Ako zistiť, čo urobí najbližší nočný beh — BEZ zápisu do eshopu a bez dotyku živých dát:** skopíruj `data/out/{decisions,order_pairings,uploaded_pairings,review_data,supplier_assignments}.json` do tmp adresára, `WEBREVIEW_OUT=<tmp>` (`WEBREVIEW_PRODUCTS` môže ukazovať na živý `data/products.csv` — číta sa len na čítanie), importuj `webreview/app.py` a poskladaj riadky cez `import_builder.new_pairing_keys` / `link_rows` / `new_order_pairing_keys` / `order_pairing_rows` + `_export_row_verdicts` (vracia `{confirmed, absent}` — `absent` = kódy, ktoré eshop v katalógu nemá a ktoré sa od #270 NEposielajú). Dostaneš presné čísla (koľko riadkov, koľko potvrdí export, koľko sa reálne odošle, v koľkých častiach) bez jediného volania `run_import`. **`data/out` je ŽIVÁ práca manažéra — nikdy doň nepíš z analýzy ani z testu** (backend testy to majú poistené v `tests/conftest.py`, ktorý prepne `WEBREVIEW_OUT`/`WEBREVIEW_PRODUCTS` na tmp ešte pred zberom testov).
...
- **Result read-back MUSÍ priradiť riadok Logu VLASTNÉMU behu (#23 → #196 → #257):** Shoptet renderuje Log NAJNOVŠÍ HORE a každý záznam začína vlastným rastúcim číslom — `#12689 26.07.2026 21:00 Upozornenie Import skončil s chybou. Spracované: 35. Upravené: 31. Zlyhanie variantov: 2.` (**presný tvar si pozri v archíve `data/out/shoptet_import_*.log` — netreba prehliadač ani prihlásenie**). „Vezmi najvrchnejší riadok odlišný od baseline" NESTAČÍ: keď do toho istého Logu píše ešte jeden import (dve automatizácie v jednej inštancii — `automation_runner.run_now` spúšťa vlákno na automatizáciu — alebo dve inštancie), prečíta sa CUDZÍ riadok. `pick_result_row(rows, baseline, expected_rows)` preto hľadá náš záznam podľa (1) čísla záznamu novšieho než baseline a (2) **`Spracované: N` == počet riadkov CSV, ktoré sme odoslali** (`preflight_csv` `total` → `_do_import` → `_read_result`; platí to na VŠETKÝCH archivovaných behoch). Nejednoznačné (dva rovnako veľké importy) alebo náš riadok neprišiel → `None` → exit 2, **nikdy sa nehádaj**. Tvrdá chyba bez sumáru (`Chyba | Číslo riadku: N - …`) sa berie ako naša len keď je JEDINÝ nový záznam.
- **`Zlyhanie variantov: N` NIE JE zlyhanie celej dávky (#257):** `Spracované: 35. Upravené: 31. Zlyhanie variantov: 2.` znamená, že Shoptet vzal všetkých 35 odoslaných riadkov a 2 varianty odmietol — 31 sa reálne zapísalo. `chunk_outcome(rc, parsed, rows_sent)` to klasifikuje ako `partial` (dávka POKRAČUJE ďalšími časťami); ako `failed` len keď `Spracované` chýba/je menšie než odoslaných riadkov, alebo zlyhalo všetko.
- **PASCA — z výstupu skriptu parsuj LEN vlastný výsledok (`result_stdout_slice`).** `scripts/shoptet_import.py` píše na stdout aj priebeh a `parse_import_log` berie PRVÚ zhodu v texte, takže parsovanie celého stdoutu prečíta počty úplne cudzieho riadku. Konkrétne to spôsobovala ozvena baseline záznamu (`[import] baseline …: #12688 … Spracované: 4.`) — appka roky hlásila počty PREDOŠLÉHO importu (to je celý symptóm #196: `processed=1/failed=1` pri reálne spracovaných 260) a `chunk_outcome` kvôli tomu videl každú čiastočne prijatú dávku ako tvrdé zlyhanie. Skript preto vypisuje baseline len ako ČÍSLO záznamu, tvrdú chybu Shoptetu dáva na stdout ako `CHYBA LOGU: …` (`hard_error_detail()` z nej vytiahne samotný dôvod) a appka parsuje výhradne `result_stdout_slice(out)`. **Testovacie stuby `run_import` musia mať reálny tvar stdoutu (`VÝSLEDOK:` …) AJ reálny POČET (`spracované={počet riadkov CSV}`, nie natvrdo `1`), inak testujú niečo, čo v prevádzke neexistuje — a „vyvrátia" kontrolu počtov pri `rc==0`.**
- **Read-back musí vyčerpať CELÉ poll okno, nie „dve zhodné čítania".** `_read_result` polluje `retries`× po `wait_s` (6×2 s). Prvá verzia ustálenia sa vracala hneď po PRVÝCH dvoch zhodných čítaniach → reálne pokryla ~2 s: cudzí import ROVNAKEJ veľkosti (dávky sú presne `IMPORT_CHUNK_ROWS`=300, čiže zhoda veľkostí je bežná, nie exotická) zapísaný medzi našou baseline a naším čítaním sa prijal ako náš, `chunk_outcome` dal `ok` a 300 kódov skončilo v `uploaded_pairings.json` — `new_pairing_keys` ich potom preskakuje NAVŽDY (tichá strata). Správne: prejsť všetky čítania, prijať len ak počas CELÉHO okna sedel PRÁVE JEDEN záznam (podľa čísla `#12689`, nie podľa textu riadku — text je nestabilný) a sedí aj pri POSLEDNOM čítaní; inak `None` → exit 2 a riadky sa jednoducho pošlú znova.
- **„Posledné čítanie" ≠ posledný `reload` — musí to byť posledné čítanie, ktoré Log NAOZAJ VIDELO.** `page.reload(wait_until="networkidle")` sa vie vrátiť skôr, než sa tabuľka Logu vykreslí; vtedy `_row_texts` vráti prázdno / len hlavičku, `pick_result_row` dá `None` a celé okno, v ktorom sa všetky reálne čítania zhodli na našom zázname, sa zahodilo ako „nečitateľné" (exit 2) — a keďže volajúci na nečitateľnom chunku robí `break`, jeden zlý render preskočil zvyšok nočných chunkov (fail-closed, dáta sa nestratia, ale manažér vidí „výsledok sa nepodarilo prečítať" pri behu, ktorý prešiel). `has_log_entries(rows)` (v `parovanie.shoptet_import`, podľa `_RESULT_ROW_RE`) rozlíši **„ešte sa nevykreslilo"** (verdikt drží posledné čítanie, ktoré tabuľku videlo) od **„vykreslené, ale nepriradiťeľné"** (ostáva fatálne). Pozor pri úprave: samotné `len(seen) == 1` nestačí — pri NESKOREJ nejednoznačnosti `pick_result_row` do `seen` nič nepridá, takže podmienka z predošlých čítaní stále platí a kredit by dostal beh, ktorý Shoptet nepotvrdil.
- **Kódovanie výstupu potomka sa MUSÍ pripnúť (`PYTHONIOENCODING=utf-8`).** `encoding="utf-8"` na `Popen` hovorí len, ako RODIČ dekóduje; potomok kóduje podľa svojho locale. Na ne-UTF-8 boxe príde `V?SLEDOK: spracovan?=35 …`, presné hľadanie `VÝSLEDOK:` minie, slice je prázdny → `processed=None` → KAŽDÝ chunk `failed` a celý nočný push ticho zomrie na jednom znaku. Preto: pripnúť env potomkovi A marker hľadať tolerantne (`V.{0,2}SLEDOK:`) — nikdy sa nespoliehať na jediný non-ASCII znak v kontrakte medzi procesmi.
- **Potvrdenie z exportu má DÁTUM SPOTREBY (`EXPORT_MAX_AGE_S`, 6 h).** Potvrdený riadok sa NEposiela a ZAPÍŠE sa ako nahraný — čiže je len taký dôveryhodný ako `data/products.csv`, ktorý obnovuje INÁ hodinová automatizácia. Keď ten sync stojí, súbor si drží staré bajty a kód medzitým zmenený v eshope by sa už nikdy neprepísal. Starší export → nepotvrdiť nič, radšej riadky poslať (idempotentné). Chýbajúci súbor blokovať netreba — vtedy `_read_export_for_links` aj tak vráti `""`. **Export sa preto číta VNÚTRI `_export_row_verdicts`, hneď za kontrolou veku — funkcia (ani `_export_note_index` pod ňou) zámerne neprijíma predpripravené `notes=`**: akonáhle by sa dali podať zvonku (prirodzená optimalizácia „prečítaj export raz a podaj ďalej"), kredit by vznikol z bajtov, ktorých vek nikto neoveril, a táto poistka by ticho zmizla.
- **`rc == 0` samo o sebe NIE JE dôkaz, že prešli všetky riadky.** Invariant „rc 0 ⇒ processed == odoslané" žije v `result_exit_code` (iný modul), takže `chunk_outcome` si počty overuje sám: `ok` len keď `Spracované` == odoslaným riadkom, alebo keď počet chýba (to je `--dry-run`). Inak by sa rc-0 výsledok s nesúhlasnými počtami zapísal ako celá nahraná dávka.
- **Log NIKDY nepovie, KTORÝ riadok zlyhal — jediný dôkaz na úrovni riadku je vlastný export eshopu.** Preto sa z `partial` časti NESMIE zapísať žiaden konkrétny kód ako nahraný; `webreview/app.py::_export_note_index()` / `_export_row_verdicts(rows)` porovnajú `internalNote` z `data/products.csv` (stĺpec 257, hodinový sync) s tým, čo by sme poslali — presná zhoda = dôkaz, kód sa zaznačí a UŽ SA NEPOSIELA. Odmietnuté riadky sa posielajú znova a sú vidieť na karte (`confirmed_in_export`, `rejected`, „Shoptet odmietol N z M riadkov"). Prázdny export nepotvrdí nič → všetko sa idempotentne pošle znova (bezpečný smer — opak supplier write-backu, ktorý musí fail-closed, lebo prepisuje).
