...
- **`GET /api/search?q=`** = server-side cez `catalog_index.search_catalog` — **NIE holý substring** (to bolo „hrozné": `"percussion bunda"`→0, `"nôž"`(noz) našiel `ponožky`). Dotaz sa rozdelí na SLOVÁ, **KAŽDÉ musí sedieť** (AND, nezávisle od poradia); na názve **prefix slova** (celé slovo=3, prefix=2, NIE substring v strede → `noz` sedí na `nôž`, nie na `ponožky`), kód=substring, dodávateľ=prefix; **ranking** (presný kód +100, presný názov +50, súvislý v názve +5) → zoradené, top 50. `<2` znaky → `[]`. `name_words` je precomputed v `build_catalog_index`. `_search_result` dolepí `idx`/`our_url` + **`price`/`stock`/`state`** (agregované z exportu pri stavbe indexu — `state` cez kanonický `export_helpers.current_of`, min naprieč variantmi) + **`paired_url`** (aktuálne good/manual rozhodnutie; GRUBE display-normalizované na .de; decisions load RAZ per request). Klik na produkt v appke otvorí **plnú `renderCard`** (nie holý panel). **POZOR: Shoptet `stock` môže byť ZÁPORNÝ (backorder, napr. -150)** — počet ks zobrazuj len keď `> 0`, inak šum.
