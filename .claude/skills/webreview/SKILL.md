...
**Dva agregáty na tomto tabe majú ZÁMERNE iný rozsah — rozhodni sa vedome (#206/#207).** Zobrazený súčet „Σ spolu X ks" (`groupQtyTotals`) sa ráta nad VŠETKÝMI riadkami dodávateľa (aj skrytými #205) — manažér objednáva PRODUKT, nie viditeľné riadky; naopak text do schránky (`orderCopyText`) berie presne to, čo skupina ZOBRAZUJE (so zapnutým #205 teda len nevybavené) — do mailu dodávateľovi sa nesmie dostať množina riadkov, ktorú pred odoslaním nevidel. Kopírovaný text navyše agreguje per produkt+VEĽKOSŤ (dve veľkosti = dva riadky objednávky) a púšťa len `safeHttpUrl` odkaz; prázdne stĺpce sa VYNECHÁVAJÚ, nevypchávajú `—` (v maile by to čítal dodávateľ ako pokyn). Množstvo ber cez `orderQty(o)` — fallback na `1` presne ako zobrazenie riadku (`o.qty || '1'`), inak riadok „1 ks" ráta ako 0.
