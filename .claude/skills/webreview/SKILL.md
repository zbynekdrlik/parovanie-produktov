...
- **Záznam sa NIKDY nemaže** (ohraničený počtom riadkov × 4). Mazanie pri „usadení" otvára GENERAČNÚ dieru: oneskorenec z predošlej dávky pristane na zázname, ktorý medzitým vytvoril NOVÝ klik, a otrávi jeho baseline → fantómový príznak je späť (`test_a_straggler_from_an_older_burst_cannot_poison_a_later_click`).
...
Preusporiadanie ODPOVEDÍ voči poradiu requestov ostáva klientsky neriešiteľné a nerieši sa.

**Zápis MIMO `saveOrderFlag`, čo mení tie isté riadky, MUSÍ vstúpiť do tej istej evidencie (revízia PR #233).** Hromadné „označiť skupinu objednané" (`markGroupOrdered`) šlo mimo — takže pomalý per-riadkový toggle, ktorý zlyhal AŽ POTOM, vrátil riadok na neobjednaný a vyhlásil chybu, hoci server ho drží ako objednaný. Po úspechu volaj `noteFlagConfirmed(field, map, keys, on)` (bumpne `seq` → prebije letiace per-riadkové zápisy, nastaví `confirmed`). Každá nová cesta, čo píše ten istý flag, musí spraviť to isté.
