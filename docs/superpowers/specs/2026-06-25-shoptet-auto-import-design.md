...
3. **Login:** headless Chromium (Playwright), prihlásenie údajmi zo súboru.
   Overí, že je naozaj prihlásený (inak stop).
4. **Import:** Produkty → Import → nahrá CSV → nastaví **kódovanie UTF-8**,
   **„nahradiť prázdne hodnoty" = VYPNUTÉ**, **párovať podľa Kódu** → spustí.
5. **Výsledok:** prečíta záložku **Log** → naparsuje a vypíše skutočný výsledok
   (Spracované / Upravené / Zlyhania variantov). Uloží screenshot + textový log do
   `data/out/shoptet_import_<čas>.{png,log}` na audit. Ak Shoptet hlási zlyhania
   → návratový kód ≠ 0 a jasná správa.
...
- `--dry-run` overí celý reťazec okrem ostrého zápisu.
- Po importe sa číta **skutočný** výsledok — nie „tváril sa OK".
- Heslo nikdy do gitu; logy/screenshoty do `data/out/` (gitignored).
- Komplexné logovanie každého kroku (login, upload, nastavenia, výsledok).
